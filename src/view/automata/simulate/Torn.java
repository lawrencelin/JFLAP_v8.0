...
public class Torn {
	 ...
	 */
	public static float paintSymbolString(Graphics2D g, SymbolString symbols, float x,
		...
		if (align == BOTTOM)
			y -= height;
		if (selected > symbols.size())
			selected = symbols.size();
		...
		g2.clip(torn);
		float dx = 0.0f;
		String string = symbols.toString();
		...
		if (selected >= 0 && !left && right) {
			...
			SymbolString before = symbols.subList(0, selected), after = symbols
					.subList(selected);
			String bString = before.isEmpty() ? "" : before.toString()+ " ",
					aString = after.isEmpty() ? "" : after.toString();
			
			double bLength = metrics.getStringBounds(bString, g2).getWidth();
			double aLength = metrics.getStringBounds(aString, g2).getWidth();
			
			float aStart = bLength > width / 2.0f ? width / 2.0f
					: horizontalPadding + (float) bLength;
			float bStart = aStart - (float) bLength;
			g2.setColor(Color.gray);
			g2.drawString(bString, x + bStart, y + toBaseline);
			g2.setColor(Color.black);
			g2.drawString(aString, x + aStart, y + toBaseline);
		} else {
			if (selected >= 0) {
				SymbolString before = symbols.subList(0, selected);
				SymbolString after = symbols.subList(selected, selected+1);
				String bString = before.isEmpty() ? "" : before.toString()+" ";
				String aString = after.isEmpty() ? "" : after.toString();
				
				double l = metrics.getStringBounds(bString,
						g2).getWidth();
